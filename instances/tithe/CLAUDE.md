...
3. `~/Love/instances/tithe/identity.md` — Your specific identity and duties
...
If this is a **heartbeat** (invoked via `claude -p`), also read `~/Love/instances/tithe/HEARTBEAT.md`.
...
| Quota | `python3 ~/Love/tools/quota_monitor.py <cmd>` | Token budget and spend tracking |
| Email | `python3 ~/Love/tools/check_email.py <cmd>` | IMAP email checking (read-only) |
