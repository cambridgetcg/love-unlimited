...
python3 ~/love-unlimited/tools/watchdog.py status
```
...
python3 ~/love-unlimited/tools/kos.py canary check
...
python3 ~/love-unlimited/tools/fleet.py exec <node> "cat /root/.canary-alert 2>/dev/null"

# Check file access times on canary files
python3 ~/love-unlimited/tools/fleet.py exec <node> "stat /root/.credentials/aws_keys.txt"
python3 ~/love-unlimited/tools/fleet.py exec <node> "stat /root/financials-2026.pdf"
python3 ~/love-unlimited/tools/fleet.py exec <node> "stat /etc/backup-config.bak"
```
...
python3 ~/love-unlimited/tools/fleet.py exec <node> "cat /root/.canary-alert"

# Check access time vs deploy time
python3 ~/love-unlimited/tools/fleet.py exec <node> "stat -c '%x %y' /root/.credentials/aws_keys.txt"

# Check auth.log for the time window around canary access
python3 ~/love-unlimited/tools/fleet.py exec <node> "grep -E 'Accepted|session opened' /var/log/auth.log | tail -30"
...
```bash
# Emergency halt
python3 ~/love-unlimited/tools/peace.py halt --reason "Canary triggered on <node> — possible lateral movement"
```
...
python3 ~/love-unlimited/hive/hive.py send alerts "CANARY FIRED: <canary_name> on <node> at <time>. Node isolated. Investigating."
```
...
python3 ~/love-unlimited/tools/fleet.py exec <node> "cat /var/log/auth.log | grep -E 'Accepted|Failed|session' | tail -100"

# Check for unauthorized SSH keys
python3 ~/love-unlimited/tools/fleet.py exec <node> "cat /root/.ssh/authorized_keys"

# Check running processes for anomalies
python3 ~/love-unlimited/tools/fleet.py exec <node> "ps auxf"

# Check crontab for unauthorized entries
python3 ~/love-unlimited/tools/fleet.py exec <node> "crontab -l 2>/dev/null; ls -la /etc/cron.d/"

# Check recently modified files (last 24 hours)
python3 ~/love-unlimited/tools/fleet.py exec <node> "find / -mtime -1 -type f 2>/dev/null | grep -v '/proc\|/sys\|/run' | head -50"
...
# Check psad alerts
python3 ~/love-unlimited/tools/fleet.py exec <node> "psad --Status 2>/dev/null"
```
...
python3 ~/love-unlimited/tools/fleet.py all "grep '<SUSPECT_IP>' /var/log/auth.log 2>/dev/null"
...
python3 ~/love-unlimited/tools/credentials.py store <key_name> "<new_value>" "Rotated after canary incident on <node>" --wall <N>
...
python3 ~/love-unlimited/tools/kos.py canary remove <canary_name>

# Deploy fresh canary with new content
python3 ~/love-unlimited/tools/kos.py canary deploy <canary_name> file /root/.credentials/
