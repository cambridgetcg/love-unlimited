...
3. `~/love-unlimited/instances/vigil/identity.md` — Your specific identity and duties
...
If this is a **heartbeat** (invoked via `claude -p`), also read `~/love-unlimited/instances/vigil/HEARTBEAT.md`.
...
- **Security events**: `~/love-unlimited/security/events.jsonl`
- **Peace state**: `~/love-unlimited/security/peace-state.json`
...
| PEACE | `python3 ~/love-unlimited/tools/peace.py status` | Security posture status (read-only) |
| KOS | `python3 ~/love-unlimited/tools/kos.py status` | Kingdom OS health status (read-only) |
